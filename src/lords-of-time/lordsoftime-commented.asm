...
; 0062  0063    2       (Variable) Current input string character pointer
; 0064  0064    1       (Variable) Count of number of characters written to the current screen row - zp_chars_on_current_line
; 0068  0069    2       (Variable) Random seed
; 006A  006A    1       (Variable) Temporary cache for a value
; 006B  006B    1       (Variable) Temporary cache for a value
; 006C  006C    1       (Variable) Temporary cache for a value

; 006E  006E    1       (Variable) Holds the opcode's 7th bit value which is used to 
;                       determine if there are one or two bytes TODO
; 006F  006F    1       (Variable) Holds the opcode's 7th bit value which is used to 
;                       determine if there are one or two bytes TODO
;                        zp_opcode_6th_bit
; 0080  0081    2       (Variable) A-code current instruction pointer - zp_a_code_ptr_lsb/msb
; 0082  0083    2       (Constant) Start address of encoded dictionary
;
; 1220                  (Encoded) Dictionary
; 5C20                  A-code
...
INCLUDE "lordsoftime-dictionary.asm"

;1B00
.data_exits_start
INCLUDE "lordsoftime-exits.asm"

;1DB7 - 1DFF Junk bytes (included in the file above)

;1E00
.list1_object_initial_locations
INCLUDE "lordsoftime-list1-initial-object-locations.asm"

;1E9C
;Never read - unsure of their purpose
INCLUDE "lordsoftime-junk-bytes-1.asm"

;1F40
.list3_object_score_values
INCLUDE "lordsoftime-list3-object-score-values.asm"

;1E9C
;Never read - unsure of their purpose
INCLUDE "lordsoftime-junk-bytes-2.asm"

;2083
.data_messages_start
INCLUDE "lordsoftime-messages.asm"

;5A0B
.data_common_word_fragments_start
INCLUDE "lordsoftime-common-word-fragments.asm"

;5C20 (4A20)
.data_a_code_start
        ; -----------------------------------------------
        ; Lords of Time A-code - this is the game logic 
        ; written in Level 9's special language called
        ; A-code. 
        ; -----------------------------------------------

INCLUDE "lordsoftime-a-code.asm"
...
.data_list_location_lookup_table
        ; List 0 is never used by The Lords of Time
        ; and $DB2B is an MOS address
        EQUB    $2B, $BD

;734B
        ; List 1 - Initial object location
        ;  
        ; Bug/feature
        ; ~~~~~~~~~~~
...
;734D
        ; List 2 - Current object location
        ;
        ; Bug/feature
        ; ~~~~~~~~~~~
...
;734F
        ; List 3 - Value of object. Either $00, $01, or $03
...
.fn_generate_random_seed
        ...
        ;
        ; ((((((oldSeed << 8) + 0x0A) - oldSeed) << 2) + oldSeed) + 1) & 0xFFF
        ; -------------------------------------------
...
.fn_load_game
        ...
        ; Sets Y to $00
        LDY     #HI(zp_load_file_parameter_block)

.set_param_block_lsb_and_call_osfile
        LDX     #LO(zp_load_file_parameter_block)
        JSR     OSFILE
...
.fn_create_file_parameter_block
        ...
        STA     zp_load_file_parameter_block

        LDA     #HI(data_save_game_file_name)
        STA     zp_load_file_parameter_block+1
        ...
        ; $003C = $FF
        LDA     #$00
        STA     zp_load_file_parameter_block+2

        LDA     #$04
        STA     zp_load_file_parameter_block+3

        LDX     #$FF
        STX     zp_load_file_parameter_block+4
        STX     zp_load_file_parameter_block+5

        ; Don't set the file execution address
        ; as it doesn't get executed so 
        ; $003D - $0040 are undefined here

        ; Set the file length to $0400
        ...
        ; $0044 = $FF
        LDA     #$00
        STA     zp_load_file_parameter_block+10

        LDA     #$04
        STA     zp_load_file_parameter_block+11
        STX     zp_load_file_parameter_block+12
        STX     zp_load_file_parameter_block+13

        ; Set the end address to $06FF
        ; (and incorrectly pad the other two
        ; variables $47/$48)        
        ; $0045 = $00
        ; $0046 = $04
        ; $0047 = $04
        ; $0048 = $04
        LDA     #$FF
        STA     zp_load_file_parameter_block+14
        LDA     #$06
        STA     zp_load_file_parameter_block+15
        STX     zp_load_file_parameter_block+16
        STX     zp_load_file_parameter_block+17
...
.fn_cmd_jump
        ...
        ; Retrieve the 16-bit a-code offset from the jump
        ; table - this is specified in operand1 and operand2
        ; and the function stores this is in $72/$73
        ; This is the start position of the jump table
        JSR     fn_get_a_code_opcode_operands

        ; Get the jump table offset into the A-code
        ; This is the position in the jump table
...
.set_jump_table_offset
        ...
        ; Update the A-code pointer LSB to the jump address LSB
        STA     zp_a_code_ptr_lsb
...
.fn_list_handler
        ...
        ; If listvv:
        ;
        ;    <operand1> - (2 bytes) indicates which list item to set 
        ;    <operand2> - (1 byte)  variable number to get value of and
        ;                 to set list item to
        ;         
        ...
        ; Defaults to listc1v so this is really
...
.check_locations_for_inverse_direction
        ...
        ; testing that first o see if it's possible
        ; to get from there back to the player's current
        ; location using the inverse direction
        LDA     #$01
        STA     zp_inverse_direction_location_counter
...
.to_location_did_not_match
        ...
        LDX     zp_inverse_direction_location_counter
        ...
        ; one didn't allow it
        INX
        STX     zp_inverse_direction_location_counter
...
.fn_clear_variables
        ...
        ; Clear the variables
        ; 
        ; 1. Pointlessly go around a loop 10 times
        ; 2. Cleat the cache
        ;
        ; Bug/feature
        ; ~~~~~~~~~~~
...
.fn_clear_stack
        ...
        ; Bug/feature
        ; ~~~~~~~~~~~
        ; Why set the stack to $180 and not $1FF?
        ; ----------------------------------------------------
...
.fn_read_player_input
        ...
        LDX     #$00
        STX     zp_input_parameter_block_buffer_lsb

        ; Set the MSB for the string buffer location £00xx
        LDX     #$00
        STX     zp_input_parameter_block_buffer_msb

        ; Save the maximum line length based on the passed Y value
        STY     zp_input_parameter_block_max_length

        ; Set the minimum acceptable character value ($20 for space)
        LDX     #$20
        STX     zp_input_parameter_block_min_char_value

        ; Set the maxmimum acceptable character value ($7F for delete)
        LDX     #$7F
        STX     zp_input_parameter_block_max_char_value

        ; Set the MSB of the parameter block
        LDY     #HI(zp_input_parameter_block_buffer_lsb)

        ; Set the LSB of the parameter block
        LDX     #LO(zp_input_parameter_block_buffer_lsb)
...
.fn_get_data_checksum
        ...
        LDA     data_checksum_start_page_msb ; $12
        STA     zp_temp_cache_2
...
.subtraction_inner_loop
        ...
        ; zp_digit_in_position_flag will then be set so all subsequent
